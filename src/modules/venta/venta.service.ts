...
export class VentaService {
  ...
  async create(createVentaDto: CreateVentaDto, user: User): Promise<Venta> {
    ...
    if (found) {
      throw new ConflictException(
        'Ya existe una venta registrada a ese chasis',
      );
    }
    const vendedor: Vendedor = await this.vendedorRepository.findOne({
      where: { id: createVentaDto.idVendedor, status: Status.ACTIVO },
    });
    if (!vendedor) {
      throw new NotFoundException(
        'El vendedor introducido no existe o esta deshabilidado',
      );
    }
    const modelo: Modelo = await this.modeloRepository.findOne({
      where: { id: createVentaDto.model, status: Status.ACTIVO },
    });
    if (!modelo) {
      throw new NotFoundException(
        'El modelo introducido no existe o esta deshabilidado',
      );
    }
  ...
  async findAllByUser(user: User): Promise<VentaEncuesta[]> {
    const totalEcuestas: Encuesta[] = await this.encuestaRepository.find({
    ...
    for (let i = 0; i < ventas.length; i++) {
      ...
      if (totalEcuestas.length != ventas[i].cuestionarios.length) {
        for (let j = 0; j < ventas[i].cuestionarios.length; j++) {
          var found = totalEcuestas.find(
            (x) => x.id == ventas[i].cuestionarios[j].encuesta.id,
          );
          if (found) {
  ...
  async update(
    ...
    const vendedor: Vendedor = await this.vendedorRepository.findOne({
      where: { id: updateVentaDto.idVendedor, status: Status.ACTIVO },
    });
    if (!vendedor) {
      throw new NotFoundException(
        'El vendedor introducido no existe o esta deshabilidado',
    ...
    const modelo: Modelo = await this.modeloRepository.findOne({
      where: { id: updateVentaDto.model, status: Status.ACTIVO },
    });
    if (!modelo) {
      throw new NotFoundException(
        'El modelo introducido no existe o esta deshabilidado',
  ...
  async reassign(
    id: string,
    updateVentaDto: ReassignVentaDto,
    user: User,
  ): Promise<Venta> {
    const found: Venta = await this.ventaRepository.findOne({
      where: { id: id },
    ...
    const userNew: User = await this.userRepository.findOne({
      where: { id: updateVentaDto.iduser, status: Status.ACTIVO },
    });
    if (!userNew) {
      throw new NotFoundException(
        'El Usuario introducido no existe o esta deshabilidado',
      );
    }
    const userActual: User = await this.userRepository.findOne({
      where: { id: found.iduser, status: Status.ACTIVO },
    });
  ...
  async createMasivo(
    ...
    const ventas: Venta[] = [];
    createVentaDto.forEach(async (element) => {
      const found: Venta = await this.ventaRepository.findOne({
        where: {
          vehiculo: {
