...
export class ReporteEncuestaService {
  ...
  async create(
    createReporteEncuestaDto: CreateReporteEncuestaDto,
  ): Promise<ReadReporteEncuestaDto> {
    const readReporteEncuestaDto: ReadReporteEncuestaDto =
      new ReadReporteEncuestaDto();
    ...
    all.forEach((element) => {
      if (element.cuestionarios.length === 0) {
        ...
        if (
          element.cuestionarios.find(
            (x) => x.encuesta.id === createReporteEncuestaDto.idEncuesta,
          ) == null
        ) {
          readReporteEncuestaDto.pendientes.push(element);
        }
